...
export const jobSeekerDeleteApplication = catchAsyncError(async(req,res,next)=>{
    ...
    if(!application){
        return next (new ErrorHandler("Oops ! Application Not Fond",404))
    }
...
export const postApplication = catchAsyncError(async(req,res,next)=>{
    ...
    const allowedFormats = ["image/jpg","image/webp","image/png"];
    if(!allowedFormats.includes(resume.mimetype)){
        return next (new ErrorHandler("Invalid file type.Please upload in a  png or jpg or webp format",400))
    }
    ...
    if(!cloudinaryResponse ||cloudinaryResponse.error){
        console.error("Cloudinary error",cloudinaryResponse.error || "Unknown Clodinary Error");
    ...
    if(!name || !email || !coverLetter || !phone || !address || !applicantID|| !employerID || !resume){
        return next (new ErrorHandler("please fil all the field ",400))
    }
